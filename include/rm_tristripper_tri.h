...
inline rm_void rm_tristripper_tri_remove_from_list(rm_tristripper_tri* tri, rm_tristripper_tri** head);
...
{
	rm_assert(neighbour_index < 3, "Neighbour index is limited to 0...2.");
	tri->link_state |= (((rm_tristripper_tri_link_state)1) << neighbour_index);
}
...
inline rm_void rm_tristripper_tri_save_link_state(rm_tristripper_tri* tri)
{
	tri->link_state = (tri->link_state << 3) | (tri->link_state & 7);
}
...
inline rm_void rm_tristripper_tri_add_tunnel_state(rm_tristripper_tri* tri, rm_size neighbour_index)
{
	...
	//Append the state:
	tri->tunnel_state = (tri->tunnel_state << 2) | ((rm_tristripper_tri_tunnel_state)neighbour_index);
}
