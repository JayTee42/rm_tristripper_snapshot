...
static rm_void rm_tristripper_verifier_log_missing_tris(const rm_tristripper_tri_occurrence_hashmap* occurrences, rm_tristripper_tri_key key, rm_tristripper_tri_occurrence occurrence, rm_void* context);
...
rm_bool rm_tristripper_verify(const rm_tristripper_verifier* verifier, const rm_tristripper_strip* strips, rm_size strips_count, rm_bool log_errors)
{
	...
	{
		//Oh no :( find the first missing triangle to provide a helpful error message:
		if (log_errors)
		{
